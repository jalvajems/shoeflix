...
const createDateFilter = (filter, startDate, endDate) => {
  ...
  const loadDashboard = async (req, res) => {
    // Define filter at the beginning so it's accessible in both try and catch
    ...
    try {
      ...
      res.render('dashboard', {
        ...
        totalPages: 0,
        currentPage: 1,
        selectedFilter: filter || 'daily', // Now filter is defined
        startDate: startDate || '',
        endDate: endDate || '',
...
const generateSalesReport = async (req, res) => {
    try {
      ...
      const orders = await Order.aggregate([
        { $match: { ...dateFilter, status: 'Delivered' } }, // Only include delivered orders
        { $unwind: '$orderedItems' },
        {
          $lookup: {
            from: 'products',
        ...
        },
        {
          $group: {
            ...
            discount: { $first: '$discount' }, // Total discount applied
            couponCode: { $first: '$couponCode' }, // Coupon code used
            couponDiscount: { $first: '$couponDiscount' }, // Assuming coupon discount is stored
      ...
      const summary = await Order.aggregate([
        { $match: { ...dateFilter, status: 'Delivered' } },
        {
          $group: {
            ...
            totalDiscount: { $sum: '$discount' },
            totalCouponDiscount: { $sum: '$couponDiscount' }, // Assuming coupon discount field exists
      ...
      orders.forEach(order => {
        order.orderedItems.forEach(item => {
          worksheet.addRow({
            ...
            couponDiscount: order.couponDiscount || 0, // Add coupon discount
